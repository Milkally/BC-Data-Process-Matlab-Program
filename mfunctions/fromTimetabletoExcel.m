...
function [tt,sizeofTimetable,T1,N1,whether]=fromTimetabletoExcel(TimeTable,outputfilename)
tt=TimeTable.Date_Time;
tt1=datestr(tt);
sizeofTimetable=size(TimeTable,1);
%获得重构数据的时间数据
dat_2=datevec(tt);
yy2=dat_2(:,1);mm2=dat_2(:,2);dd2=dat_2(:,3); hh2=dat_2(:,4);
...
N1=cell(sizeofTimetable+1,size(T1,2)+4);
%第一行
N1(1,1)=T1.Properties.VariableNames(1,1);%VariableNames,see: help table
N1(1,2)={'Year'};N1(1,3)={'Month'};N1(1,4)={'Day'};N1(1,5)={'Hour'};
N1(1,6:end)=T1.Properties.VariableNames(1,2:end);
%第1-5列：完整时间，年，月，日，小时
...
%第6列
N1(2:end,6:end)=table2cell(T1(1:end,2:end));
